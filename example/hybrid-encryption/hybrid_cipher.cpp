...
HybridEncryption::HybridEncryption(const mbedtls_mpi *secret) : HybridEncryption() {
  mbedtls_mpi_copy(&g_secret_hybrid_key, secret);
}
...
HybridEncryption::HybridEncryption() {
  ...
  if ((ret = mbedtls_ssl_setup(&dummy_ssl_ctx, &dummy_ssl_cfg)) != 0) {
    fprintf(stderr, "failed to setup ssl: %d", ret);
  };
...
void HybridEncryption::initServer(ECPointBuffer pubkey) {
  ...
  if (g_secret_hybrid_key.p == NULL) {
    fprintf(stderr,"key not provisioned yet");
  }
#else
  fprintf(stderr, "*** PREDEFINED SECRET KEY IS USED ***");
  fprintf(stderr, "*** DISABLE THIS BEFORE DEPLOY ***");
  ret = mbedtls_mpi_read_string(&g_secret_hybrid_key, 16, PREDEFINED_HYBRID_SECKEY);
  if (ret != 0) {
    fprintf(stderr, "Error: mbedtls_mpi_read_string returned %d", ret);
    return;
  }
...
int HybridEncryption::secretToPubkey(const mbedtls_mpi *seckey, ECPointBuffer pubkey) {
  ...
  if (ret != 0) {
    fprintf(stderr, "Error: mbedtls_ecp_mul returned %d", ret);
    return -1;
  }
  ...
  if (buflen != 65) {
    fprintf(stderr, "ecp serialization is incorrect olen=%ld", buflen);
  }
